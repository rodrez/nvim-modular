...
local function get_tmux_sessions()
  local sessions = {}
  local cmd = M.config.tmux_command .. ' list-sessions -F "#{session_name}:#{session_windows}:#{session_attached}" 2>/dev/null'
  ...
  pickers.new(opts, {
    ...
    finder = finders.new_table {
      ...
      entry_maker = function(entry)
        local display = string.format('%d. %s (%s windows)%s', 
          entry.index, 
          entry.name, 
          entry.windows,
...
M.new_session = function(opts)
  ...
  pickers.new(opts, {
    ...
    finder = finders.new_table {
      results = directories,
      entry_maker = function(entry)
        local display_name = vim.fn.fnamemodify(entry, ':t') .. ' (' .. vim.fn.fnamemodify(entry, ':h') .. ')'
        return {
          value = entry,
